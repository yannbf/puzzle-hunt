import { styled, keyframes, useTheme } from '@storybook/theming';
import {
  ErrorMessage,
  ...
  const handleFormSubmit = useCallback(
    async (
      ...
      const response = await fetch(
        'https://5p6bidvesabtv2l2rhh2qz64ki0odxsw.lambda-url.us-east-1.on.aws',
        {
          method: 'POST',
          mode: 'cors',
          body: JSON.stringify({
            form: { name, email, townOfBirth, numberDonuts, numberDice },
            env: {
      ...
      if (onSubmit) {
        onSubmit({
          values: { name, email, townOfBirth, numberDonuts, numberDice },
          result,
        });
      }
          ...
          <Wrapper>
            ...
            <Content>
              <FormElement noValidate aria-disabled={isSubmitting ? 'true' : 'false'}>
                ...
                <FieldWrapper>
                  <Label htmlFor="townOfBirth">What town were you born in?</Label>
                  <FormikInput id="townOfBirth" name="townOfBirth">
                    {({ field }: { field: HTMLAttributes<HTMLInputElement> }) => (
                      <>
                        <Input
                          id="townOfBirth"
                          ...
                          disabled={isSubmitting}
                          type="townOfBirth"
                          aria-invalid={errors.townOfBirth ? 'true' : 'false'}
                          {...field}
                        />
                        {errors.townOfBirth && (
                          <WithTooltip
                            tooltip={<ErrorTooltip>{errors.townOfBirthTooltip}</ErrorTooltip>}
                          >
                            <ErrorWrapper>
                              <ErrorIcon icon="question" />
